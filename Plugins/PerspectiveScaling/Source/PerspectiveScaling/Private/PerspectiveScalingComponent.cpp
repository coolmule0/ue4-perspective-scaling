...
float UPerspectiveScalingComponent::FindPerspectivePoint() const
{
	...
	{
		FVector UnitLength; float Length;
		(Vertex - TraceStart).ToDirectionAndLength(UnitLength, Length);
		const FVector TraceEnd = UnitLength * GrabDistance + TraceStart;
		...
		if (GetWorld()->LineTraceSingleByChannel(HitResult, TraceStart, TraceEnd, ECC_Visibility, Params)) {
			float CenterHitDistance = (AttachedActor->GetActorLocation() - this->GetComponentLocation()).Size()* HitResult.Distance / (Vertex - this->GetComponentLocation()).Size();

			ClosestTraceHit = (CenterHitDistance < ClosestTraceHit) ? CenterHitDistance : ClosestTraceHit;
		}
	}
